...
class ResponseCache(t.Generic[Args, Value]):
    ...
    def __init__(
        ...
        *,
        ...
        timezone_sensitive: bool = sentinel,
        ...
        :param timezone_sensitive:
            If True, signals that the output of the wrapped method depends on the timezone
            guessed for the request (see :func:`viur.core.utils.guess_timezone`) and should
            be cached separately for each timezone. Set it when localized `DateBone` values
            are rendered into the response; otherwise the timezone of the first request after
            expiry is served to everyone.
        ...
        if timezone_sensitive is sentinel:
            timezone_sensitive = DEFAULT_SETTINGS.timezone_sensitive
        ...
        self.timezone_sensitive: bool = timezone_sensitive
    ...
    def get_args(
        ...
        if self.timezone_sensitive:
            res["__timezone"] = utils.guess_timezone().zone
