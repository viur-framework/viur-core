...
class Session:
    """
        ...
        - :prop:use_session_cookie is set to True by default, causing the cookie to be treated as a session cookie
            ...
            conf.viur.session_lifeTime.
        - The config variable conf.viur.session_lifeTime: Determines, how long (in seconds) a session is valid.
        ...
        - The config variables conf.viur.session_persistentFieldsOnLogin and
            conf.viur.session_persistentFieldsOnLogout lists fields, that may survive a login/logout action.
    ...
    def load(self, req: BrowseHandler):
        ...
        if cookie_key := str(req.request.cookies.get(self.cookie_name)):
            if data := db.Get(db.Key(self.kindName, cookie_key)):  # Loaded successfully
                if data["lastseen"] < time.time() - conf.viur.session_lifeTime:
    ...
    def save(self, req: BrowseHandler):
        ...
        flags = (
            ...
            f"Max-Age={conf.viur.session_lifeTime}" if not self.use_session_cookie else None,
        )
...
def start_clear_sessions():
    ...
    """
    query = db.Query(Session.kindName).filter("lastseen <", time.time() - (conf.viur.session_lifeTime + 300))
