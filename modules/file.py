...
class fileBaseSkel(TreeLeafSkel):
	...
	width = numericBone(escr="Width", indexed=True, searchable=True)
...
class File(Tree):
	...
	def upload(self, node = None, *args, **kwargs):
		...
		:param node: Optional parent node in the file repository to upload files to.
		"""
		...
		try:
			...
			for upload in self.getUploads():
				...
				try:
					...
					height = width = 0
					logging.info("some error occurred while trying to fetch the image header with dimensions")
				...
				fileSkel.setValues({
					...
					"parentdir": str(nodeSkel["key"].value) if nodeSkel else None,
					"parentrepo": nodeSkel["parentrepo"].value if nodeSkel else None,
					"weak": False,
					"width": width,
					"height": height
				})
			...
			for r in res:
				logging.info("Upload successful: %s (%s)" % (r["name"].value, r["dlkey"].value))
	...
	def download(self, blobKey, fileName = "", download = "", *args, **kwargs):
		...
		if download == "1":
			fname = "".join([c for c in fileName
			                        if c in string.ascii_lowercase + string.ascii_uppercase + string.digits + ".-_"])
...
def doDeleteWeakReferences(timeStamp, cursor = None):
	...
	q = skelCls().all().filter("weak =", True).filter("creationdate <", datetime.strptime(timeStamp,"%d.%m.%Y %H:%M:%S"))

	if cursor:
		q.cursor(cursor)

	for skel in q.fetch(99):
