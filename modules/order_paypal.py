	...
	class PayPalHandler:
		...
		def getPayURL(self, token):
			if self.API_ENDPOINT != 'https://api-3t.paypal.com/nvp':  # Sandbox
				return "https://www.sandbox.paypal.com/webscr?cmd=_express-checkout&token=%s&RETURNURL=%s&CANCELURL=%s" % (
				token, urllib.quote_plus(self.returnurl), urllib.quote_plus(self.cancelurl))
			else:  # Live System!
				return "https://www.paypal.com/webscr?cmd=_express-checkout&token=%s&RETURNURL=%s&CANCELURL=%s" % (
				token, urllib.quote_plus(self.returnurl), urllib.quote_plus(self.cancelurl))
		...
		def SetExpressCheckout(self, amount):
			...
			response = urlfetch.fetch(self.API_ENDPOINT, params_string.encode("UTF-8"), "POST",
									  deadline=10).content.decode("UTF-8")
			response_token = ""
	...
	def doPayPal(self, token, PayerID, *args, **kwargs):
		...
		if res["ACK"].lower() == u"success":
			...
			return self.orderHandler.render.getEnv().get_template(
				self.orderHandler.render.getTemplateFileName("paypal_okay")).render()
		else:
			return self.orderHandler.render.getEnv().get_template(
				self.orderHandler.render.getTemplateFileName("paypal_failed")).render()
