...
class Sofort(object):
	...
	def getSofortURL(self, orderID):
		...
		conf["sofort"]["userid"], conf["sofort"]["projectid"], float(order["price"]), str(order.key()),
		str(order.key()), conf["sofort"]["projectpassword"])
	...
	def sofortStatus(self, *args, **kwargs):
		...
		if hashlib.sha512(hashstr.encode("utf-8")).hexdigest() != kwargs["hash"]:
			logging.error("RECEIVED INVALID HASH FOR sofort (%s!=%s)" % (
			hashlib.sha512(hashstr.encode("utf-8")).hexdigest(), kwargs["hash"]))
			return ("INVALID HASH")
	...
	def doSofort(self, *args, **kwargs):
		return self.orderHandler.render.getEnv().get_template(
			self.orderHandler.render.getTemplateFileName("sofort_okay")).render()

	doSofort.exposed = True

	def sofortFailed(self, *args, **kwargs):
		return self.orderHandler.render.getEnv().get_template(
			self.orderHandler.render.getTemplateFileName("sofort_failed")).render()
