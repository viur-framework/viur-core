# -*- coding: utf-8 -*-
from server import db, utils, request, tasks
...
def keyFromArgs( f, userSensitive, evaluatedArgs, path, *args, **kwargs ):
	"""
...
def wrapCallable(f, urls, userSensitive, evaluatedArgs, maxCacheTime):
	"""
	...
	def wrapF( self, *args, **kwargs ):
		...
		key = keyFromArgs( f, userSensitive, evaluatedArgs, path, *args, **kwargs )
		if not key:
...
def enableCache( urls, userSensitive=False, evaluatedArgs=[], maxCacheTime=None  ):
	"""
	...
	"""
	return lambda f: wrapCallable( f, urls, userSensitive, evaluatedArgs, maxCacheTime )
