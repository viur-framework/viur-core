...
class BrowseHandler(object):  #webapp.RequestHandler
	...
	def processRequest(self, path, *args, **kwargs):  # Bring up the enviroment for this request, handle errors
		...
		if sharedConf["viur.disabled"] and not (
				users.is_current_user_admin() or "HTTP_X_QUEUE_NAME".lower() in [x.lower() for x in
																				 os.environ.keys()]):  # FIXME: Validate this works
			...
			self.response.out.write(
				tpl.safe_substitute({"error_code": "503", "error_name": "Service unavailable", "error_descr": msg}))
			return
