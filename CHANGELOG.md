...
- `import logging` must stay behind other imports in `__init__.py` (#573)
...
- Moved `utils.projectID` to `conf["viur.instance.project_id"]` (#562)
- Moved `utils.isLocalDevelopmentServer` to `conf["viur.instance.is_dev_server"]` (#562)
- Changed default values for `conf["viur.render.json.downloadUrlExpiration"]` and `conf["viur.render.json.downloadUrlExpiration"]` to None (#552)
- Fixed HTTP reason phrase not be translated according to RFC2616 (#549)
- Dependency bumped `viur-datastore` from `1.3.2` to `1.3.4`
- Dependency bumped `pillow` from 9.1.1 to 9.3.0 (#560)
- Added `is_deferred` flag to currentRequest (#556)
- Added fine-grained `required`-flag to specify requirement for specific languages (#511)
- Added linter GitHub Action workflow (#554, #545, #543, #539, #540, #504)
- Removed obsolete `skey == ""` check from add/edit methods (#548)
- Refactored `getReferencedBlobs` and `getSearchTags` (#528)
- Fixed `utils.sanitizeFileName` to quote non-ASCII-characters in filename (caused error 500) (#547)
