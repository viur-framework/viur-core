...
- Implement global `conf` variable as `Conf`-class (#567)
- Add `ModuleConfig` (#551)
- Provide `indexed`-flag in bone structures (#568)
- Move `utils.projectID` and `utils.isLocalDevelopmentServer` into config (#562)
- Default disable `viur.render.json.downloadUrlExpiration` and `viur.render.json.downloadUrlExpiration` (#552)
- Don't translate name for HTTP Reason Phrase (#549)
- Updating to use `viur-datastore==1.3.3` (#563)
...
- Remove obsolete `skey == ""` check from add/edit methods (#548)
- Rewrite `getReferencedBlobs` and `getSearchTags` (#528)
- Improve `utils.sanitizeFileName` to quote non-ASCII-characters in filename (caused error 500) (#547)
...
- Improve check for `viur.maxPostParamsCount` (#526)
- Redefining `__reserved_keywords` (#529)
- Remove default logging handler which caused redundant logging output on local dev app server (#521)
- Use `skel.refresh()` in updateRelations (#524)
- Cleaning up and renaming CallDeferred (#513)
- Get project zone from metadata.google.internal (#519)
...
- Improving `SpatialBone` bounds checking and use `float` instead of `int` (#507)
- Improving `fieldPath` in for precise error reporting (#505)
- Extending `KeyBone` to check given key's existence (#497)
- Convert TranslationsKey (#501)
- Add `maxLength` for `StringBone` (#500)
- Enhancing max-value for `SortIndexBone` (#499)
- Re-enabling translations (#491)
- Add valid mime types to the bone structure (#498)
